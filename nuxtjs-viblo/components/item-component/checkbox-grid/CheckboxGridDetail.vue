          ...
          <table>
            <thead>
              <tr>
                <th></th>
                <th
                  v-for="(column, columnIndex) in tableColumns.slice(1)"
                  :key="columnIndex"
                >
                  <label>
                    {{ column.label }}
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [{}, { label: "Cột 1" }],
      tableData: [["Hàng 1", false]],
    };
  },
  mounted() {
  ...
  methods: {
    ...
    addRow() {
      this.tableData.push(
        Array(this.tableColumns.length).fill("Hàng" + this.tableData.length)
      );
    },
    addColumn() {
